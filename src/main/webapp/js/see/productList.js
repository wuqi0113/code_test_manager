layui.use(['laydate', 'laypage', 'layer', 'table', 'carousel', 'upload', 'element', 'slider'], function() {
    ...
    table.render({
        elem: '#productList'
        // ,height: 'full-80'
        ,url:ctx+ '/authproduct/productList' //数据接口
        ,title: '产品表'
        ,page: true //开启分页
        ,even:true
        ,limit:20
        ,limits:[20,30,50]
        ,toolbar: '#productToolBar' //开启工具栏，此处显示默认图标，可以自定义模板，详见文档
        ,totalRow: true //开启合计行
        ,cols: [[ //表头
            ...
            ,{field: 'productSize', title: '产品规格', width: 200}
        ...
        switch(obj.event){
            case 'save':
                ...
                var index = layui.layer.open({
                    title : "添加产品",
                    type : 2,
                    content : ctx+"/authproduct/toAddPage",
                ...
                $(window).resize(function(){
                    // $(".layui-layer-content").css("height","1000px")
                    // $("iframe").css("height","100%")
                    layer.full(index);
                })
                layer.full(index);
    ...
    table.on('tool(productList)', function(obj){ //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
        ...
        if(layEvent === 'batchList'){
            // layer.msg('查看操作');
            location.href = ctx+"/authproduct/toBatchList?id="+data.id;

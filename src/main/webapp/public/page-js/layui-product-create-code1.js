...
layui.use('table', function(){
  ...
  table.render({
    ...
    ,cols: [[
      ...
      ,{field:'status', title: '签发状态', width:100}
  ...
  var $ = layui.$, active = {
    ...
    table.on('tool(LAY_table_code)', function(obj){ //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
        ...
        if(layEvent === 'codeInfo'){
            // location.href="authproduct/batchSignMsg?proAddress="+$("#proAddress").val()+"&num="+data.num;
            $.ajax({
                url:'authproduct/batchSignMsg',
                data:{num:data.num,proAddress:$("#proAddress").val()},
                dataType:'json',
                success:function (d) {
                    if (d.code==0){

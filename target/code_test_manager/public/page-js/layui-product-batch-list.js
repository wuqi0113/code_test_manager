...
layui.use('table', function(){
  ...
  //方法级渲染
  table.render({
    elem: '#LAY_table_product_batch'
    ,width: 1100
    ,url: 'authproduct/getProductBatchList'
    ,cols: [[
          {type:'checkbox'}
          ...
          ,{field:'operate', title: '操作', width:300,align:'center', toolbar: '#batchDemo'}
    ]]
  ...
  var $ = layui.$, active = {
    ...
    //监听行工具事件
    table.on('tool(LAY_table_product_batch)', function(obj){ //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
        ...
        } else if(layEvent === 'createCode'){
            location.href = 'authproduct/toCreateCodePage/'+data.sname;
        }else if(layEvent === 'proList'){
            var index = layui.layer.open({
                title : "产生防伪码",
                type : 2,
                content : "authproduct/createCode/"+data.sname,
                success : function(layero, index){
                    layui.layer.tips('点击X返回列表', '.layui-layer-setwin .layui-layer-close', {
                        tips: 3
                    });

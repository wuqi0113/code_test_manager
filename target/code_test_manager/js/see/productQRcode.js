...
layui.use('table', function(){
    ...
    //方法级渲染
    table.render({
        elem: '#productQRCodeList'
        // ,width: 930
        ,url: ctx+'/authproduct/getSignMsgList'
        ,toolbar: '#qrcodeToolBar' //开启工具栏，此处显示默认图标，可以自定义模板，详见文档
        ,totalRow: true //开启合计行
        ,cols: [[
            ...
            ,{field:'signId', title: '签名ID',width:200}
    ...
    //监听行工具事件
    table.on('tool(productQRCodeList)', function(obj){ //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
        var data = obj.data //获得当前行数据
            ,layEvent = obj.event; //获得 lay-event 对应的值
        if(layEvent === 'codeInfo'){
            location.href=ctx+"/authproduct/batchSignMsg?proAddress="+$("#proAddress").val()+"&num="+data.num+"&sname="+$("#sname").val()+"&signId="+data.signId;
            /*$.ajax({
...
//监听头工具栏事件
    table.on('toolbar(productQRCodeList)', function(obj){

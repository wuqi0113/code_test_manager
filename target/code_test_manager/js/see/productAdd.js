...
layui.use(['form','layer','jquery','laydate', 'laypage',  'table', 'carousel', 'upload', 'element', 'slider'], function() {
    ...
    form.on("submit(saveProduct)",function(d){
        ...
        $.ajax({
            type: "get",
            url: ctx+"/authmanage/checkManager/158Yh5k9C6K8sibvn9zxYhtaKvYMjLRtcE",
        ...
        $.ajax({
            type: "POST",
            url: ctx+"/authproduct/addPro",
            data:$("#arf").serialize(),
            success:function(data3){
                console.info(data3)
                if(data3.code==0){
                    invite(data3.authProduct.id,data3.sysManage.manageAddr,data3.sysManage.address,index);
                }else{
                    top.layer.close(index);
